namespace API.Controllers
{
    ...
    public class PorukaController : ControllerBase
    {
        ...
        public async Task<ActionResult<Poruka>> PoslataPoruka(int porukaID)
        {
        ...
        public async Task<ActionResult<ICollection<Poruka>>> Razgovor(int korisnikID) 
        {
        ...
        public async Task<ActionResult<bool>> AutoriPoruka(int korisnikID) 
        {
            var korisnik = await inalog.KorisnikPoID(korisnikID);
            if (korisnik == null)
                return NotFound("Ne postoji traženi korisnik");
        ...
        public async Task<ActionResult<Poruka>> PosaljiPoruku(int korisnikID, string tekst) 
        {
            var korisnik = await inalog.KorisnikPoID(korisnikID);
            if (korisnik == null)
                return NotFound("Ne postoji traženi korisnik.");
        ...
        public async Task<ActionResult<Poruka>> PrepraviPoruku(int porukaID, string tekst) 
        {
            if (tekst.Equals(string.Empty))
                return BadRequest("Mora se uneti tekst poruke.");
        ...
        public async Task<ActionResult<string>> ObrisiPoruku(int porukaID) 
        {
        ...
        public async Task<ActionResult<string>> ObrisiRazgovor(int korisnikID) 
        {
            var korisnik = await inalog.KorisnikPoID(korisnikID);
            if (korisnik == null)
                return NotFound("Ne postoji traženi korisnik.");
