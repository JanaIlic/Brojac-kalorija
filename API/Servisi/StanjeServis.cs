namespace API.Servisi
{
    public class StanjeServis : IStanje
    {
        ...
        public StanjeServis(DataContext dc, IHttpContextAccessor hca) 
        { 
            context = dc;
        ...
        {
            return await context.Stanja.Where(s => (s.korisnikID == funkcije.PrijavljenID())).ToListAsync();    
        }

        public async Task<Stanje> StanjePoIDu(int id)
        {
            var stanje = await context.Stanja.Where(s => ((s.ID == id) && (s.korisnikID == funkcije.PrijavljenID()))).FirstOrDefaultAsync();
            if (stanje == null)
                return null;
        ...
        {
            var stanje = await context.Stanja.Where(s => s.korisnikID == funkcije.PrijavljenID()).OrderBy(s => s.Datum).FirstOrDefaultAsync();
            if (stanje == null)
                return null;
        ...
        public async Task<Stanje> AktuelnoStanje() 
        {
            ...
            else
            {
                ...
                stanje.korisnik = await context.Korisnici.FindAsync(stanje.korisnikID);
                stanje.korisnik.DatumRodjenja = (await context.Korisnici.FindAsync(stanje.korisnikID)).DatumRodjenja;
        ...
        {
              var stanje = await context.Stanja.Where(s => s.Datum.Equals(datum) && (s.korisnikID == funkcije.PrijavljenID())).FirstOrDefaultAsync();
              if (stanje == null)
              {
                  stanje = (await Stanja()).First();
        ...
        public async Task<Stanje> UpisiStanje(double visina, double tezina, double nt)
        {
            ...
            stanje.korisnikID = funkcije.PrijavljenID();
            stanje.korisnik = await context.Korisnici.FindAsync(stanje.korisnikID);
            stanje.korisnik.Pol = (await context.Korisnici.FindAsync(stanje.korisnikID)).Pol;
            stanje.korisnik.DatumRodjenja = (await context.Korisnici.FindAsync(stanje.korisnikID)).DatumRodjenja;
            ...
            {
                var danas = await context.Dani.Where(dan => dan.korisnikID == stanje.korisnikID 
            ...
            await context.Stanja.AddAsync(stanje);
        ...
        public async Task<Dan> PromeniDnevniRezultat(double promena) 
        {
            ...
            var izvestaj = await context.Izvestaji.Where(i => i.danID == danas.ID).FirstOrDefaultAsync();
            if (izvestaj != null) 
            {
